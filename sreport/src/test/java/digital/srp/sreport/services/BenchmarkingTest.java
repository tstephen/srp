...
public class BenchmarkingTest {
    ...
    private static final BigDecimal totalProcurementCo2e = new BigDecimal(
            "3000000");

    private static final BigDecimal totalCoreCo2e = new BigDecimal("4000000");

    private static final BigDecimal totalCommissioningCo2e = new BigDecimal(
            "5000000");

    private static final BigDecimal totalCommunityCo2e = new BigDecimal(
            "6000000");

    private static final BigDecimal totalProcurement2017Co2e = new BigDecimal(
            "7000000");
    ...
    public void testFullBenchmarking() {
        ...
        svc.benchmarkPerMetric(PERIOD, rtn, totalCo2e, totalCoreCo2e,
                totalCommissioningCo2e, totalProcurementCo2e,
                totalProcurement2017Co2e, totalCommunityCo2e, totalEnergyCo2e,
                totalTravelCo2e, floorArea, metricName);

        // check inputs unchanged
        assertEquals(totalProcurementCo2e,
                rtn.answer(PERIOD, Q.TOTAL_PROCUREMENT_CO2E).get()
                        .responseAsBigDecimal());

        assertCurrentBenchmarks(rtn);
        assertEquals(new BigDecimal("10000000"),
                rtn.answer(PERIOD, Q.TOTAL_ENERGY_CO2E_BY_FLOOR).get()
                        .responseAsBigDecimal()
                        .setScale(0, RoundingMode.HALF_UP));
        assertEquals(new BigDecimal("20000000"),
                rtn.answer(PERIOD, Q.TOTAL_TRAVEL_CO2E_BY_FLOOR).get()
                        .responseAsBigDecimal()
                        .setScale(0, RoundingMode.HALF_UP));
    }
    ...
    private SurveyReturn getSurveyIncEmissions() {
        SurveyReturn rtn = new SurveyReturn().applicablePeriod(PERIOD)
                .org(ORG_CODE);
        rtn.getAnswers()
                .add(new Answer().applicablePeriod(PERIOD)
                        .question(Q.TOTAL_PROCUREMENT_CO2E)
                        .response(totalProcurementCo2e));
        rtn.getAnswers().add(new Answer().applicablePeriod(PERIOD)
                .question(Q.TOTAL_CORE_CO2E).response(totalCoreCo2e));
        rtn.getAnswers()
                .add(new Answer().applicablePeriod(PERIOD)
                        .question(Q.TOTAL_COMMISSIONING_CO2E)
                        .response(totalCommissioningCo2e));
        rtn.getAnswers().add(new Answer().applicablePeriod(PERIOD)
                .question(Q.TOTAL_COMMUNITY_CO2E).response(totalCommunityCo2e));
        rtn.getAnswers()
                .add(new Answer().applicablePeriod(PERIOD)
                        .question(Q.TOTAL_PROCUREMENT_2017_CO2E)
                        .response(totalProcurement2017Co2e));
        rtn.getAnswers().add(new Answer().applicablePeriod(PERIOD)
    ...
    private void assertCurrentBenchmarks(SurveyReturn rtn) {
        assertEquals(new BigDecimal("30000000"),
                rtn.answer(PERIOD, Q.TOTAL_PROCUREMENT_CO2E_BY_FLOOR).get()
                        .responseAsBigDecimal()
                        .setScale(0, RoundingMode.HALF_UP));
        assertEquals(new BigDecimal("40000000"),
                rtn.answer(PERIOD, Q.TOTAL_CORE_CO2E_BY_FLOOR).get()
                        .responseAsBigDecimal()
                        .setScale(0, RoundingMode.HALF_UP));
        assertEquals(new BigDecimal("50000000"),
                rtn.answer(PERIOD, Q.TOTAL_COMMISSIONING_CO2E_BY_FLOOR).get()
                        .responseAsBigDecimal()
                        .setScale(0, RoundingMode.HALF_UP));
        assertEquals(new BigDecimal("60000000"),
                rtn.answer(PERIOD, Q.TOTAL_COMMUNITY_CO2E_BY_FLOOR).get()
                        .responseAsBigDecimal()
                        .setScale(0, RoundingMode.HALF_UP));
        assertEquals(new BigDecimal("70000000"),
                rtn.answer(PERIOD, Q.TOTAL_PROCUREMENT_2017_CO2E_BY_FLOOR).get()
                        .responseAsBigDecimal()
                        .setScale(0, RoundingMode.HALF_UP));
        assertEquals(new BigDecimal("80000000"),
                rtn.answer(PERIOD, Q.TOTAL_CO2E_BY_FLOOR).get()
                        .responseAsBigDecimal()
                        .setScale(0, RoundingMode.HALF_UP));
    }
