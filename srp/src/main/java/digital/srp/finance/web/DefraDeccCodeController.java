...
import digital.srp.finance.repositories.DefraDeccCodeRepository;
import digital.srp.model.Event;

@Controller
@RequestMapping("/{tenantId}/defraDeccCodes")
public class DefraDeccCodeController {

    protected static final Logger LOGGER = LoggerFactory
            .getLogger(DefraDeccCodeController.class);
    ...
    @Autowired
    protected DefraDeccCodeRepository codeRepo;
    ...
    public final @ResponseBody List<Event> importMultipleFiles(
        ...
        for (MultipartFile resource : resourceFile) {
            try {
                ...
                DefraDeccCode[] entities = om.readValue(is,
                        DefraDeccCode[].class);
                for (DefraDeccCode entity : entities) {
                    if (entity.getCategory() == null) {
                        String msg = String.format("No category in %1$s",
                                entity.getCode());
                        LOGGER.error(msg);
